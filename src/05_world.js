  ...
  if (detail < 1.6) {
    ...
    for (let f = 0; f < b.floors; f++) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        if (b.tower && detail > 5 && f > 2 && (c === 2 || c === 4 || c === 6 || c === 8) && f % 2 === (c === 2 || c === 6 ? 0 : 1)) { const bw2 = wW + 16, bx2 = x - 8, by2 = y + wH; ctx.fillStyle = '#2b3550'; ctx.fillRect(bx2, by2, bw2, 6); ctx.fillStyle = 'rgba(0,0,0,.4)'; ctx.fillRect(bx2, by2 + 6, bw2, 5); ctx.fillStyle = '#4a5a7a'; ctx.fillRect(bx2, by2 - 22, 2, 22); ctx.fillRect(bx2 + bw2 - 2, by2 - 22, 2, 22); for (let i = 1; i < 6; i++) ctx.fillRect(bx2 + bw2 * i / 6, by2 - 20, 1, 20); ctx.fillRect(bx2, by2 - 23, bw2, 2); if (hash2(c + 11, f + b.id) < .3) { ctx.fillStyle = '#2a5a3a'; ctx.beginPath(); ctx.arc(bx2 + 6, by2 - 6, 4, 0, TAU); ctx.fill(); } if (hash2(c + 5, f * 3) < .12) { ctx.fillStyle = '#e8e2d2'; ctx.fillRect(bx2 + bw2 * .5, by2 - 24, 1.5, 6); ctx.fillRect(bx2 + bw2 * .5 - 5, by2 - 24, 12, 1.5); } }
